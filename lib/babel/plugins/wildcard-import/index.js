			...
			ImportDeclaration(code, state) {
				...
				if (fs.existsSync(fullImportPath) && fs.lstatSync(fullImportPath).isDirectory()) {
					...
					if (
						...
						Object.keys(state.opts.changeExtensions).length > 0
					) {
					...
					if (!hasIndexFiles) {
						if (node.specifiers.length === 0) {
							files.forEach((file) => {
								code.insertAfter(t.importDeclaration([], t.stringLiteral(path.join(importSource, file))));
							});
						} else {
									...
									files.forEach((file) => {
										...
										code.insertBefore(
											t.importDeclaration(
												...
												t.stringLiteral(path.join(importSource, file))
											)
										);
									...
									if (matchingFile) {
										code.insertBefore(
											t.importDeclaration(
												...
												t.stringLiteral(path.join(importSource, matchingFile))
											)
										);
									} else {
